...
class PerformanceProfiler:
    """
    Performance profiler for detailed model analysis.
    
    Provides profiling of model components and bottleneck identification.
    """
    ...
    def profile_model(
        ...
        # Overall profiling
        predictor = DCALPredictor(self.model, self.config, self.device)
        ...
        for _ in range(num_iterations):
            start_time = time.time()
            result = predictor.predict_pair(test_data[0][0], test_data[0][1])
            end_time = time.time()
            
            inference_times.append(end_time - start_time)
            preprocessing_times.append(result['inference_time'])
        ...
        if profile_memory and self.device == 'cuda':
            results['memory'] = self._profile_memory(test_data[0])
        
        # Layer profiling
        if profile_layers:
            results['layers'] = self._profile_layers(test_data[0])
        
        self.profiling_results = results
        return results
    
    def _profile_memory(self, test_sample: Tuple[Any, Any]) -> Dict[str, Any]:
        ...
        # Load model
        model_memory = torch.cuda.memory_allocated(self.device) - baseline
        ...
        result = predictor.predict_pair(test_sample[0], test_sample[1])
        ...
        return {
            ...
            'total_memory': torch.cuda.memory_allocated(self.device),
            'max_memory': torch.cuda.max_memory_allocated(self.device)
        }
    
    def _profile_layers(self, test_sample: Tuple[Any, Any]) -> Dict[str, Any]:
        """Profile individual layers."""
        layer_times = {}
        
        # Hook function to measure layer times
        def hook_fn(name):
            def hook(module, input, output):
                ...
                # This is a simplified profiling - in practice, you'd need
                # more sophisticated timing mechanisms
                hook_fn.times[name].append(time.time())
            return hook
        ...
        # Run inference
        predictor = DCALPredictor(self.model, self.config, self.device)
        predictor.predict_pair(test_sample[0], test_sample[1])
        ...
        if hasattr(hook_fn, 'times'):
            for name, times in hook_fn.times.items():
                layer_times[name] = {
                    'calls': len(times),
    ...
    def generate_profile_report(self) -> str:
        ...
        report = []
        report.append("DCAL Twin Faces Verification - Performance Profile")
        report.append("=" * 60)
        report.append("")
        ...
        if 'inference' in self.profiling_results:
            ...
            report.append(f"  Average Time: {inf_results['avg_time']:.4f}s")
            report.append(f"  Std Time: {inf_results['std_time']:.4f}s")
            report.append(f"  Min Time: {inf_results['min_time']:.4f}s")
            report.append(f"  Max Time: {inf_results['max_time']:.4f}s")
            report.append(f"  Throughput: {inf_results['throughput']:.2f} samples/s")
            report.append("")
        
        # Memory profiling
        if 'memory' in self.profiling_results:
            mem_results = self.profiling_results['memory']
            report.append("Memory Usage:")
            report.append(f"  Model Memory: {mem_results['model_memory'] / 1024**2:.2f} MB")
            report.append(f"  Inference Memory: {mem_results['inference_memory'] / 1024**2:.2f} MB")
            report.append(f"  Total Memory: {mem_results['total_memory'] / 1024**2:.2f} MB")
            report.append(f"  Max Memory: {mem_results['max_memory'] / 1024**2:.2f} MB")
            report.append("")
        
        # Layer profiling
        if 'layers' in self.profiling_results:
            layer_results = self.profiling_results['layers']
            report.append("Layer Profiling:")
            for name, stats in layer_results.items():
                report.append(f"  {name}: {stats['calls']} calls, {stats['total_time']:.4f}s total")
